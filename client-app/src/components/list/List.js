      ...
      <Table style={{ border: '1px solid black' }}>
        <TableHead>
          <TableRow>
            ...
            <TableCell className={classes.head}>Цена (плацкарт, KZT)</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
              ...
              <Fragment key={index}>
                ...
                {result.trains.map((train, idx) => (
                  <TableRow key={idx}>
                    ...
                    <TableCell>
                      {format(
                        new Date(train.departureStationDepartureDate),
                        'dd-MM-yyyy HH:mm',
                      )}
                    </TableCell>
                    ...
                    <TableCell>
                      {format(
                        new Date(train.arrivalStationArrivalDate),
                        'dd-MM-yyyy HH:mm',
                      )}
                    </TableCell>
                    <TableCell>{train.arrivalStationName}</TableCell>
                    {idx === 0 && (
                      <TableCell rowSpan={result.trains.length}>
